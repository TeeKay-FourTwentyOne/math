 ...
 *
 * Single-threaded but runs R replicas at fixed temperatures with periodic
 * replica exchange swaps. Simpler and deadlock-free.
 *
...
#define STEPS_PER_REPLICA 200  /* SA steps per replica per round */
...
/* Swap RNG */
static unsigned long long swap_rng[4];
...
static inline unsigned long long rng_next_r(unsigned long long *s) {
...
static inline unsigned long long rng_next(Replica *r) {
    return rng_next_r(r->rng_s);
}

static void rng_seed_arr(unsigned long long *s, unsigned long long seed) {
    s[0] = seed ^ 0x9E3779B97F4A7C15ULL;
    s[1] = seed ^ 0x6C62272E07BB0142ULL;
    s[2] = seed ^ 0xBF58476D1CE4E5B9ULL;
    s[3] = seed ^ 0x94D049BB133111EBULL;
    for (int i = 0; i < 20; i++) rng_next_r(s);
}

static void rng_seed(Replica *r, unsigned long long seed) {
    rng_seed_arr(r->rng_s, seed);
}
...
static inline double swap_rng_double(void) {
    return (rng_next_r(swap_rng) >> 11) * 0x1.0p-53;
}
...
static void rebuild_d11_lists(Replica *r) {
    r->d11_in_cnt = 0; r->d11_out_cnt = 0;
...
static void d12_swap_to_mem(Replica *r, int elem) {
    ...
    r->d12_nmem[ni] = last; r->d12_nmem_idx[last] = ni;
    r->d12_ncnt--;
...
static void d12_swap_to_nmem(Replica *r, int elem) {
    ...
    r->d12_mem[mi] = last; r->d12_mem_idx[last] = mi;
    r->d12_cnt--;
...
static void init_replica(Replica *r, int d11_size, int seed, int rid) {
    rng_seed(r, (unsigned long long)seed * 1000003ULL + 42 + (unsigned long long)rid * 999983ULL);
...
static inline void sa_step(Replica *r) {
    ...
    if (rr < 22) {
        ...
        int rp = r->d11_in[ri], ap = r->d11_out[oi];
        ...
        if (dc <= 0 || rng_double(r) < exp(-(double)dc / fmax(r->temperature, 1e-10))) {
            ...
            r->pair_in_d11[rp] = 0; r->pair_in_d11[ap] = 1;
            rebuild_d11_lists(r);
        } else {
        ...
        int rem = r->d12_mem[ri], add_el = r->d12_nmem[ai];

        r->D12[rem] = 0; r->D12[add_el] = 1;
        int rem_t = mod_neg[rem], add_t = mod_neg[add_el];
        r->D12T[rem_t] = 0; r->D12T[add_t] = 1;
        ...
        if (dc <= 0 || rng_double(r) < exp(-(double)dc / fmax(r->temperature, 1e-10))) {
            ...
            r->D12[rem] = 1; r->D12[add_el] = 0;
            r->D12T[rem_t] = 1; r->D12T[add_t] = 0;
            update_delta_inc(r->delta_12, r->D12, add_el, rem);
            update_delta_inc(r->delta_12T, r->D12T, add_t, rem_t);
        }
    } else {
        ...
        if (dc <= 0 || rng_double(r) < exp(-(double)dc / fmax(r->temperature, 1e-10))) {
            r->current_cost = new_cost;
            d12_swap_to_nmem(r, rem1); d12_swap_to_nmem(r, rem2);
            d12_swap_to_mem(r, add1); d12_swap_to_mem(r, add2);
        } else {
...
static int solve_pt(int d11_size, int seed, int max_iter) {
    ...
    rng_seed_arr(swap_rng, (unsigned long long)seed * 314159ULL + 271828ULL);
    ...
    int num_rounds = max_iter / (STEPS_PER_REPLICA * NUM_REPLICAS);

    for (int round = 0; round < num_rounds; round++) {
        /* Run STEPS_PER_REPLICA on each replica */
        for (int ri = 0; ri < NUM_REPLICAS; ri++) {
            Replica *r = &replicas[ri];
            for (int s = 0; s < STEPS_PER_REPLICA; s++) {
                sa_step(r);
                total_steps++;

                if (r->current_cost < global_best_cost) {
                    global_best_cost = r->current_cost;
                    memcpy(best_D11, r->D11, sizeof(int) * M);
                    memcpy(best_D12, r->D12, sizeof(int) * M);
                }

                if (r->current_cost == 0) {
                    int v12_cost = verify_v1v2(r);
                    if (v12_cost == 0) {
                        clock_gettime(CLOCK_MONOTONIC, &ts_now);
                        double elapsed = (ts_now.tv_sec - ts_start.tv_sec) +
                                       (ts_now.tv_nsec - ts_start.tv_nsec) * 1e-9;
                        printf("  SOLUTION at round=%d step=%d (%.1fs), replica=%d, T=%.5f\n",
                               round, total_steps, elapsed, ri, r->temperature);
                        memcpy(best_D11, r->D11, sizeof(int) * M);
                        memcpy(best_D12, r->D12, sizeof(int) * M);
                        global_best_cost = 0;
                        return 0;
                    } else {
                        r->current_cost = v12_cost;
        ...
        for (int i = parity; i < NUM_REPLICAS - 1; i += 2) {
            ...
            if (delta_beta * delta_cost >= 0 || swap_rng_double() < exp(delta_beta * delta_cost)) {
                double tmp = replicas[i].temperature;
                replicas[i].temperature = replicas[i + 1].temperature;
                replicas[i + 1].temperature = tmp;
                swap_accepts++;
            }
        }

        /* Logging */
        if (total_steps % 2000000 < STEPS_PER_REPLICA * NUM_REPLICAS) {
            clock_gettime(CLOCK_MONOTONIC, &ts_now);
            double elapsed = (ts_now.tv_sec - ts_start.tv_sec) +
                           (ts_now.tv_nsec - ts_start.tv_nsec) * 1e-9;
            ...
            int min_cold = 999;
            for (int i = 0; i < NUM_REPLICAS; i++) {
                if (replicas[i].temperature < 0.05 && replicas[i].current_cost < min_cold)
                    min_cold = replicas[i].current_cost;
            }

            printf("    %dM: gbest=%d cold=%d swap=%.2f (%.0f it/s)\n",
                   total_steps / 1000000, global_best_cost, min_cold, swap_rate,
                   (double)total_steps / elapsed);
            fflush(stdout);
        }
    }
...
int main(int argc, char **argv) {
    ...
    int max_iter = argc > 3 ? atoi(argv[3]) : 100000000;
